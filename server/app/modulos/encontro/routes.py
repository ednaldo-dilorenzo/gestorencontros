...
@encontro_bp.route("/<hashid:id_encontro>/equipes/<hashid:id_equipe>/casal/<hashid:id_casal>", methods=["PATCH"])
def editar_equipe_encontro_casal(id_encontro, id_equipe, id_casal):
    coordenador = request.form.get("coordenador", type=lambda v: v.lower() == "true")
    aceito = request.form.get("aceito", type=lambda v: v.lower() == "true")

    equipe_encontro_casal_atual = encontro_service.buscar_equipe_encontro_casal(
        id_encontro=id_encontro, id_equipe=id_equipe, id_casal=id_casal
    )
    equipe_encontro_alterado = EquipeEncontroCasal()
    equipe_encontro_alterado.id_encontro = id_encontro
    equipe_encontro_alterado.id_equipe = id_equipe
    equipe_encontro_alterado.id_casal = id_casal
    equipe_encontro_alterado.coordenador = coordenador
    equipe_encontro_alterado.aceito = aceito
...
@encontro_bp.route("/<hashid:id_encontro>/equipes/<hashid:id_equipe>", methods=["GET"])
def buscar_casais_equipe(id_encontro, id_equipe):
    ...
    return [
        {
            "id": equipe_encontro.casal.hashid,
